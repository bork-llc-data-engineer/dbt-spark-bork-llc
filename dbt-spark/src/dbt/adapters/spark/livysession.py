...
DEFAULT_POLL_WAIT = 1
USED_LIVY_SESSIONS: Set[str] = set()
...
class LivyConnection:
    ...
    def __init__(
        ...
        verify_ssl_certificate: Optional[bool],
        session_manager: Optional["LivyConnectionManager"] = None
    ) -> None:
        self.connect_url: str = connect_url
        ...
        self.session_manager: Optional["LivyConnectionManager"] = session_manager
...
class LivyConnectionManager:
    ...
    def connect(
        ...
        data: Dict[str, Any] = {"kind": "sql", "conf": session_params}

        # 👇 НОВОЕ: идентифицируем сессию через Livy proxyUser
        if user:
            data["proxyUser"] = user
        ...
        if self.livy_global_session is None:
            ...
            existing_id = self._find_existing_session(
                connect_url, auth, headers, verify_ssl_certificate, user
            )
            if existing_id is not None:
                logger.debug(f"Attaching to existing livy session: {existing_id}")
                self.livy_global_session.session_id = existing_id
            else:
            ...
            if existing_id is not None:
                logger.debug(f"Re-attaching to existing livy session: {existing_id}")
                self.livy_global_session.session_id = existing_id
            else:
                self.livy_global_session.create_session(data)
        else:
        ...
        # 👇 ВАЖНО: помечаем, что эта сессия занята в этом процессе dbt
        if self.livy_global_session.session_id is not None:
            USED_LIVY_SESSIONS.add(self.livy_global_session.session_id)
    ...
    def delete_session(self) -> None:
        if self.livy_global_session is not None:
            sid = self.livy_global_session.session_id
            self.livy_global_session.delete_session()
            if sid is not None:
                USED_LIVY_SESSIONS.discard(sid)
    ...
    def _find_existing_session(
        ...
        try:
            ...
            logger.debug(f"Unable to list Livy sessions for reuse: {ex}")
            return None
        ...
        for s in sessions:
            # Берём только sql-сессии
            if s.get("kind") != "sql":
                continue

            state = s.get("state")
            if state not in ("idle", "busy", "starting", "not_started"):
                continue

            sid = str(s["id"])

            # Уже кем-то занята в этом процессе dbt → пропускаем
            if sid in USED_LIVY_SESSIONS:
                continue

            # Фильтр по пользователю (если Livy его возвращает)
            proxy_user = s.get("proxyUser") or s.get("owner")
            if proxy_user and user and str(proxy_user) != str(user):
                continue
